...
def get_master_map():
    ...
    for _, row in df_info.iterrows():
        f, l, n = str(row.get('상품코드', '')).strip(), str(row.get('Lab No', '')).strip(), str(row.get('상품명', '')).strip()
        info = {'formal': f if f else l, 'name': n, 'lab_no': l}
        keys = {extract_digits(f), extract_digits(l), f, l}
        for k in keys:
            if not k: continue
            if k not in mapping or (is_formal_code(info['formal']) and not is_formal_code(mapping[k]['formal'])):
                mapping[k] = info
    return mapping
...
def prepare_image_for_ai(img, mode):
    ...
    if mode == "🦓 패턴 중심(흑백)": img = img.convert("L").convert("RGB")
    elif mode == "🎨 컬러 중심(블러)": img = img.filter(ImageFilter.GaussianBlur(radius=15))
    else: img = ImageOps.autocontrast(img, cutoff=1)
    return img
...
if input_file:
    ...
    with c2: 
        search_mode = st.radio("🔎 검색 기준", ["🎨 컬러+패턴", "🦓 패턴 중심(흑백)", "🎨 컬러 중심(블러)"], horizontal=True)

    # 🚀 [복구] 세부 검색 옵션 (Expander)
    with st.expander("⚙️ 세부 보정 (조명, 회전, 밝기)", expanded=is_photo):
        ...
        with col_ex2:
            ...
            if st.button("↩️ 사진 90도 회전"):
                ...
                st.rerun()
        with col_ex3:
            brightness = st.slider("☀️ 밝기 보정", 0.5, 2.0, 1.0, 0.1, disabled=not is_photo)
            sharpness = st.slider("🔪 선명도 보정", 0.0, 3.0, 1.5, 0.1, disabled=not is_photo)
    ...
    with col_sel2:
        if st.button("⏹️ 전체 선택 (스캔용)", use_container_width=True):
    ...
    if len(st.session_state['points']) == 4:
        ...
        with col_crop1: st.image(final_crop, caption="최종 분석 대상", width=300)
        with col_crop2:
